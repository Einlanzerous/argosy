...
5. **A/V desync on the transcode remux path — real, NOT environmental** → **ARGY-84**. Video copied + non-AAC audio (TrueHD/DTS) re-encoded to AAC → audio runs a touch behind the (smooth) video. **Reproduced on the phone's built-in speakers, not Bluetooth**, so it isn't output-device latency. Hits the *common* path (any non-AAC audio). Likely server-side audio/video timestamp misalignment (re-encoded-audio PTS/priming vs copied video), possibly worse with non-zero `startAt`; could also be an ExoPlayer A/V-sync interaction with the growing playlist.
